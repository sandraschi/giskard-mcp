...
const BACKEND_NAME: &str = "giskard-mcp-backend.exe";
const BACKEND_PORT: u16 = 11056;
const BACKEND_TAG: &str = "giskard-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "GISKARD_MCP_PORT";
const ENV_HOST: &str = "MCP_HOST";
const ENV_TAURI: &str = "GISKARD_TAURI";
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    thread::spawn(move || {
        ...
        log_line(&app_health, &format!("Backend health check FAILED on port {BACKEND_PORT} after 30 attempts"));
