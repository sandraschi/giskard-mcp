...
if (Test-Path $specFile) {
    Push-Location $Root
    # Patch fastmcp to not crash on missing metadata
    ...
    Write-Host "  WARNING: spec file not found - using existing backend exe if present" -ForegroundColor DarkYellow
}
...
$nsisDir = "$PSScriptRoot\src-tauri\target\release\bundle\nsis"
if (Test-Path $nsisDir) { Copy-Item "$nsisDir\*-setup.exe" "$distDir\" -Force }

# Clean stale shadow exe that fools backend.rs resolution
Remove-Item -Force "$PSScriptRoot\src-tauri\target\release\${RepoName}-backend.exe" -ErrorAction SilentlyContinue
